...
hash_t MerkleTree::hash(const hash_t &a, const hash_t &b) {
  ...
  iroha::sha3_256(output.data(), input.data(), input.size());

  return output;
}

hash_t MerkleTree::hash(std::vector<uint8_t> &data) {
  hash_t output;
  iroha::sha3_256(output.data(), data.data(), data.size());
  return output;
}

hash_t MerkleTree::hash(uint8_t *data, size_t size) {
  hash_t output;
  iroha::sha3_256(output.data(), data, size);
  return output;
}
...
size_t MerkleTree::last_block_begin() const { return leafs_ - 1; }
size_t MerkleTree::last_block_end() const { return i_current_; }
